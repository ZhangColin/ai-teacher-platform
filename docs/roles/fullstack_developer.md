...
2. **识别问题并按 P0/P1/P2 分类（参考 5.0 节）**：
   - **阅读设计文档**：完整阅读架构设计文档，理解技术契约
   - **识别问题**：识别设计文档中未明确的技术细节、实现方式等
   - **分类问题**：将所有疑问按 P0/P1/P2 分类（参考 5.0 节的问题分类与判断标准）
     - **P0（必须讨论）**：技术决策依赖、设计问题、核心功能阻塞
     - **P1（可假设）**：有业界惯例、不影响功能实现、后续可调整
     - **P2（后续迭代）**：边界情况、不影响MVP、实现细节
   - **处理方式**：
     - **P0问题**：必须提出讨论（带方案），达成一致后再拆分任务
     - **P1问题**：采用假设方案，写入 `docs/development/todos.md`（P1列表），继续拆分任务
     - **P2问题**：不写入代码，写入 `docs/development/todos.md`（P2列表），继续拆分任务
   - **检查点**：只有所有 P0 问题都解决后，才能进入下一步（拆分任务）
...
4. **任务拆分规则（最小可交付原则）**：
   ...
   - **粒度标准**：任务粒度应该适中，既不能太细（如"创建一个文件"），也不能太粗（如"完成用户登录功能"）

5. **任务格式**：
   ```
   任务 1：实现登录API（POST /api/v1/auth/login）
   任务 2：实现登录页面（LoginPage.vue）
   任务 3：实现用户管理API（GET/POST /api/v1/admin/users）
   ```
