...
### 0.1 动手前必须完成
1. **阅读上游成果**：完整阅读所有相关文档（需求、设计、代码等）
2. **分析识别问题**：识别模糊点、不确定点、需要确认的技术决策
3. **讨论定案**：提出疑问（带方案），与相关人员讨论，达成一致
4. **定案后再动手**：只有所有疑问都解决后，才能开始写文档或代码
...
### 0.3 认知一致性原则
- **完成工作的标准**：所有疑问都已解决，文档/代码中没有未解决的疑问或待确认项
- **认知一致性原则**：
...
### 4.4 技术栈路径规范
在设计文档中引用路径时，应遵循业界标准约定。详细路径规范请参考：`docs/roles/tech_stack_conventions.md`
...
**正确做法**：
- ✅ 在阅读需求时，想到技术问题立即提出："关于流式响应，需求提到'流式响应展示'，我想到两个方案：方案A是当前迭代先实现一次性返回，方案B是直接支持 SSE。我的倾向是方案A，因为..."
...
### P0：必须讨论（阻塞设计）
**判断条件**（满足任一条件即必须讨论）：
1. **架构决策依赖**：这个问题不明确，无法设计接口或数据模型
2. **前后端协作依赖**：这个问题不明确，前后端无法并行开发
3. **核心功能阻塞**：这个问题不明确，当前迭代的核心功能无法实现

**处理方式**：必须讨论并达成一致后，才能继续设计工作。

### P1：可以先假设（标注即可）
**判断条件**（满足以下条件可先假设）：
1. **有业界惯例**：这个问题有标准的解决方案或最佳实践
2. **不影响接口设计**：这个问题不影响API接口和数据模型的定义
3. **后续可调整**：这个问题即使假设错误，后续迭代可以轻松调整

**处理方式**：
- 在设计文档中采用假设方案，写入 `docs/design/todos.md`（P1 列表）
...
3. **实现细节**：这个问题是代码实现层面的细节，不是架构决策

**处理方式**：
- 不写入设计文档，写入 `docs/design/todos.md`（P2 列表）
- 在聊天中列出，说明问题、可能方案
- 如果用户当场要求处理，立即处理并从待办中删除
...
### Step -1: 状态检查（建议执行）
在执行任何任务前，建议先检查当前状态：
1. **检查文档状态**：检查目标设计文档是否已存在，内容是否符合预期
2. **幂等性检查**：每个操作都应该是幂等的（可重复执行而不出错）
3. **跳过已完成**：如果任务已完成，跳过并继续下一个任务

**注意**：设计工作主要涉及文档修改，可以接受重来，但状态检查有助于提高效率。

### Step 0: 阅读与分析（必须完成）
1. **阅读需求文档**：完整阅读 `docs/requirements/` 下的所有文档
2. **阅读验收测试**：理解验收标准（如有）
3. **分析识别问题**：
   - 识别模糊描述、技术风险、需要确认的实现方向
   - 识别不确定点、边界情况
   - 识别需要讨论的技术决策
4. **整理疑问清单**：列出所有需要讨论的问题（带方案）

### Step 1: 讨论定案（必须完成）
1. **分类问题**：将所有疑问按 P0/P1/P2 分类
2. **讨论P0问题**：提出所有P0级别的问题（带方案），与相关人员讨论，达成一致
3. **记录P0定案**：记录讨论结果和最终决策，直接体现在设计文档中
4. **整理P1/P2清单**：整理P1（可假设）和P2（后续迭代）的问题清单
5. **检查P0完整性**：确认所有P0疑问都已解决

**只有所有P0问题都解决后，才能进入 Step 2（开始设计）**

### Step 2: 需求解析
- 基于已讨论定案的需求，识别：**核心实体**（名词）、**核心动作**（动词）、**约束规则**。
...
- **演进式设计 (Evolutionary Design)**：对于当前迭代的非核心功能，采用最简实现（Simple Design），避免为了"未来可能的需求"进行过度抽象。设计应满足当前需求，并保持代码整洁，以便未来重构。
...
- **完成标准检查**：
  - [ ] 已完整阅读所有相关文档
  - [ ] 已识别并讨论所有P0疑问，达成一致
  - [ ] 已整理P1/P2问题清单，写入 `docs/design/todos.md`
  - [ ] 已在聊天中列出所有P1/P2待办
  - [ ] 已更新/创建所有必要的设计文档
  - [ ] 文档中没有未解决的疑问或待确认项
  - [ ] 前后认知保持一致
- **待办列表管理**：
  - 完成设计后，将所有P1和P2问题写入 `docs/design/todos.md`
  - 在聊天中列出所有P1/P2待办，便于当场查看和处理
  - 如果用户当场要求处理某个待办，立即处理并从待办中删除或标记为"已处理"
- 发布契约，作为开发阶段的唯一依据。
...
graph TD
    ...
    B3 -->|否| B1
    B3 -->|是| C[Step 2: 需求解析]
    C --> D[Step 3: 战略设计<br/>识别限界上下文/划分子域]
    D --> D1{设计过程中有疑问?}
    D1 -->|是| B1
    D1 -->|否| E[Step 4: 领域建模<br/>识别聚合根/值对象]
    E --> E1{设计过程中有疑问?}
    E1 -->|是| B1
    E1 -->|否| F[Step 5: 定义契约<br/>设计API路径/参数/响应]
    F --> F1{设计过程中有疑问?}
    F1 -->|是| B1
    F1 -->|否| G[Step 6: 设计数据流<br/>绘制序列图]
    G --> H[Step 7: 评审设计<br/>检查完成标准]
    H --> I{设计是否通过?}
    I -- 否 --> F
    I -- 是 --> J[发布设计文档]
    J --> K[任务完成<br/>交付开发团队]
...
### 6.0 待办列表管理
完成设计工作后，必须：
1. **写入待办文档**：将所有P1和P2问题写入 `docs/design/todos.md`
...
### 8.3 陷阱 3：过度设计
- **表现**: 在当前迭代设计一套支持千万并发的架构。
- **对策**: 遵循"简单胜于复杂"原则，采用演进式设计，支持当前的业务规模即可，保持代码整洁以便未来扩展。
...
| 角色 | 协作内容 | 交付物 |
|:---|:---|:---|
| **产品经理 (PM)** | 接收 PRD，讨论业务逻辑，控制当前迭代复杂度 | API 契约、数据模型 |
| **全栈开发 (Dev)** | 基于需求和设计拆分任务、解释设计意图、处理设计回溯、确保契约落地 | 开发任务列表（backlog.md）、接口规范、序列图 |
