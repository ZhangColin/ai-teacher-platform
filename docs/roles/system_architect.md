...
graph TD
    ...
    D -->|否| C
    D -->|是| B
    B -->|是| E[需求解析<br/>识别核心实体/动作/约束]
    E --> F[战略设计<br/>识别限界上下文/划分子域]
    F --> G[领域建模<br/>识别聚合根/值对象/绘制领域图]
    G --> H[定义契约<br/>设计API路径/参数/响应]
    H --> I[设计数据流<br/>绘制序列图]
    I --> J[评审设计<br/>检查是否满足需求]
    J --> K{设计是否完整?}
    K -->|否| L[补充设计]
    L --> J
    K -->|是| M[发布设计文档]
...
```mermaid
graph LR
    PM[产品经理] -- 产出 PRD --> Arch[系统架构师]
    Arch -- 识别实体/定义契约 --> Design[docs/design/]
    Design -- 作为法律依据 --> Dev[开发人员]
    Dev -- 发现契约缺失 --> Arch
```
