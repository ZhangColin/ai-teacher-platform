...
class TestTitleGenerator:
    ...
    @pytest.fixture
    def title_generator(self, mock_db):
        """创建标题生成器实例"""
        with patch("src.services.title_generator.ModelProviderService"):
            return TitleGenerator(mock_db)
    ...
    def test_should_use_user_message_for_long_input(self, title_generator):
        ...
        with patch.object(title_generator, '_get_ai_client') as mock_get_client:
            ...
            # Act
            import asyncio
            title = asyncio.run(title_generator.generate_title(
                user_message, ai_response, model_provider, model_name
            ))
